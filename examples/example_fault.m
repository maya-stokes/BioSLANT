...
p.strike(1) = 45; p.dip(1) = 60; p.x0(1) = p.dx*0.5*(p.Nx-1); p.y0(1) = p.dy*0.5*(p.Ny-1); p.z0(1) = -p.E*p.tf/2; % strike in degrees E of N, dip in degrees down from horizontal (right-hand rule), elevation at x=0,y=0 (meters)
...
p.runname = 'example_fault1.mat'; %     p.runname:       Character string naming the run. If specified
